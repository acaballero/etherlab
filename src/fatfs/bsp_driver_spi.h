...
#include "../../lib/FatFs/diskio.h" //from FatFs middleware library
...
DSTATUS BSP_SPI_initialize(BYTE pdrv);

DSTATUS BSP_SPI_status(BYTE pdrv);

DRESULT BSP_SPI_read(BYTE pdrv, BYTE *buff, DWORD sector, UINT count);

#if _USE_WRITE == 1

DRESULT BSP_SPI_write(BYTE pdrv, const BYTE *buff, DWORD sector, UINT count);

#endif /* _USE_WRITE == 1 */
#if _USE_IOCTL == 1

DRESULT BSP_SPI_ioctl(BYTE pdrv, BYTE cmd, void *buff);
