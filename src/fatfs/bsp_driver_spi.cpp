...
/* We're using the port expander, so we will use the MCP23017_HandleTypeDef handlder 'hmcp02' defined in mcp23017.h */
extern SPI_HandleTypeDef SD_SPI_HANDLE;
//extern MCP23017_HandleTypeDef SD_MCP23017_HANDLE;
...
#define CS_HIGH()    {HAL_GPIO_WritePin(GPIOC, GPIO_PIN_14, GPIO_PIN_SET);}
#define CS_LOW()    {HAL_GPIO_WritePin(GPIOC, GPIO_PIN_14, GPIO_PIN_RESET);}
//#define CS_HIGH()    {setGPIOExpPin(&SD_MCP23017_HANDLE,MCP23017_PORTA, GPIOEXP_SD_CARD_CS,true);}
//#define CS_LOW()    {setGPIOExpPin(&SD_MCP23017_HANDLE,MCP23017_PORTA, GPIOEXP_SD_CARD_CS,false);}
...
BYTE xchg_spi(BYTE dat    /* Data to send */) {
    ...
    // Using bare registers instead of HAL
    //  SD_SPI_HANDLE.Instance->CR2 |= SPI_RXFIFO_THRESHOLD; // 8 bit transfer
    if ((SD_SPI_HANDLE.Instance->CR1 & SPI_CR1_SPE) != SPI_CR1_SPE) {
        SD_SPI_HANDLE.Instance->CR1 |= SPI_CR1_SPE;               //enable SPI1
    }
    *(__IO
    uint8_t *)&SD_SPI_HANDLE.Instance->DR = dat;                        // Write data to be transmitted to the SPI data register
    ...
    rxDat = *(__IO
    uint8_t *)&SD_SPI_HANDLE.Instance->DR;                        // Return received data from SPI data register
...
//
//static
//void spiselectfast(void)    /* 1:OK, 0:Timeout */
//{
//    CS_LOW();        /* Set CS# low */
//    xchg_spi(0xFF);    /* Dummy clock (force DO enabled) */
//}
...
BYTE send_cmd(        /* Return value: R1 resp (bit7==1:Failed to send) */
        ...
        DWORD arg,        /* Argument */
        bool assertCS    /* Deselect and select CS before sending command */
) {
    BYTE n, res;
    ...
    n = 10;                                /* Wait for response (10 bytes max2) */
    do {
        res = xchg_spi(0xFF);
    } while ((res & 0x80) && --n);
...
DSTATUS BSP_SPI_initialize(
    ...
    if (r == 1) {            /* Put the card SPI/Idle state */
        ...
        if (send_cmd(CMD8, 0x1AA) == 1) {    /* SDv2? */
            ...
            uint16_t res = send_cmd(ACMD41, 0, false);
            if (res <= 1) {    /* SDv1 or MMC? */
                ty = CT_SD1;
...
DRESULT BSP_SPI_ioctl(
    ...
    switch (cmd) {
        ...
        case CTRL_TRIM :    /* Erase a block of sectors (used when _USE_ERASE == 1) */
            ...
            if (BSP_SPI_ioctl(drv, MMC_GET_CSD, csd)) break;    /* Get CSD */
