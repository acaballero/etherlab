...
namespace main_board {
    ...
    void check_status() {
        ...
        bool biased = ISTX &&
                      power_amp::status == power_amp::OK &&
                      rf_coupler::info.swr < rf_coupler::HIGH_SWR &&
                      rf_coupler::info.p_for_dbm < config.max_power_dbm &&
                      battery::battery_info.status != battery::BATTERY_STATUS_LOW;

        setGPIOExpPin(&hmcp02, MCP23017_PORTA, GPIOEXP_POW_AMP_BIAS, biased, true);
    }

    void power_amp_status_callback(void *thisptr, void *args) {
        check_status();
    }

    void rf_coupler_info_callback(void *thisptr, void *args) {
        check_status();
    }

    void battery_callback(void *thisptr, void *args) {
        check_status();
    }
    ...
    void setGPIO() {
        ...
        changed = changed | setGPIOExpPin(&hmcp01, MCP23017_PORTB, GPIOEXP_LNA,
                                          !(!ISTX && config.frontend_path == radio::FRONTEND_PATH_LNA), false);
        // Frontend pass-thru path
        changed = changed | setGPIOExpPin(&hmcp01, MCP23017_PORTB, GPIOEXP_FRONT_THRU,
                                          !ISTX && config.frontend_path == radio::FRONTEND_PATH_THRU, false);
        // Frontend attenuator path
        changed = changed | setGPIOExpPin(&hmcp01, MCP23017_PORTB, GPIOEXP_FRONT_ATTENUATOR,
                                          !ISTX && config.frontend_path == radio::FRONTEND_PATH_ATT, false);

        // Set digital TX or analog RX&TX signal between the 1st and 2nd mixers
        changed = changed | setGPIOExpPin(&hmcp01, MCP23017_PORTA, GPIOEXP_ANALOG_RXTX_DIGITAL_TX_SWITCH,
                                          config.mode != DIGITAL_TX, false);
    ...
    bool _setMode(MODE mode, bool force) {
        ...
        if (force || mode != config.mode) {
            ...
            if (ISTX) {
                ...
                power_ctrl = POWCRL_PB1 | POWCRL_P5 | POWCRL_PA1 |
                             (config.modulation == SSB_LSB || config.modulation == SSB_USB ? POWCRL_PC1 : 0);
                ...
                if (config.hpa_enabled) power_amp::enable();
                rf_coupler::enable();

            } else {

                rf_coupler::disable();
                if (config.hpa_enabled) power_amp::disable();
                ...
                power_ctrl = POWCRL_PB1 | POWCRL_P5 | POWCRL_PA2 |
                             (config.modulation == SSB_LSB || config.modulation == SSB_USB ? POWCRL_PC2 : 0);
    ...
    void setModulationMode(int mod_val, bool force) {
        ...
        if (force || mod_val != (int) config.modulation) {
            ...
            switch (config.modulation) {
                case FM:
                    ...
                    changed = changed | setGPIOExpPin(&hmcp01, MCP23017_PORTB, GPIOEXP_FM_DETECTOR,
                                                      ISTX,
                                                      false); // When low, it powers up the +5v rail that goes into the FM detector board
                    changed = changed |
                              setGPIOExpPin(&hmcp01, MCP23017_PORTB, GPIOEXP_FM_MODULATOR, !(config.mode == ANALOG_TX),
                                            false);
                    changed = changed | setGPIOExpPin(&hmcp01, MCP23017_PORTB, GPIOEXP_AM_DETECTOR,
                                                      true,
                ...
                default:
                    changed =
                            changed | setGPIOExpPin(&hmcp02, MCP23017_PORTA, GPIOEXP_RSSI_LEVEL_ADAPTER, false, false);
        ...
        if (force || value != config.power_ctrl) {
            ...
            if (oneByOne) {
                ...
                uint8_t ctrl = PowControlShiftReg.getValue(),
                        new_ctrl = config.power_ctrl, mask = 1;
                ...
                for (uint8_t i = 0; i < 8; i++) {
                    ...
                    // Change the register when there's a change, and only if it is from 0 to 1 (no need to go one by one when powering off) or it's the last bit
                    if (current_ctrl != ctrl && (set || i == 7)) {

                        if (set) HAL_Delay(1); // if has changed from 0 to 1
                        current_ctrl = ctrl;
    ...
    void set_if_filter(radio::IF_FILTER fil) {
        ...
        if (new_filter != radio::if_filter) {
            ...
            if ((config.modulation == SSB_LSB || config.modulation == SSB_USB) && config.mode != DIGITAL_TX) {
                ...
                int offset = (int) (radio::if_filters[radio::if_filter].bandwidth_khz * 1000 / 2) +
                             500; // +500 to account for the skirt
