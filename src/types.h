...
enum LO_POWER {
    LO_POWER_LOW, LO_POWER_MEDIUM, LO_POWER_HIGH
};
...
struct st_freq_mem {
    ...
    char name[FREQ_MEM_NAME_SIZE +
              1] = "          "; // Must be allocated beforehand or the menu won't let increase it's size beyond the NULL char
    // Copy
...
struct st_status {
    ...
    bool operator==(const st_status &st) const {
        return modulation == st.modulation
               ...
               && band == st.band
               && agc == st.agc
               && if_filter == st.if_filter
               && f_carrier == st.f_carrier
               && filter == st.filter; // or another approach as above
    }
};
