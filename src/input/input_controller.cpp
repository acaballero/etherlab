...
void processEvent(st_inputEvent *e) {
    ...
    if (!e->time_us) e->time_us = HAL_GetTick();
    ...
    if (e->type == INPUT_EVENT_TYPE_TOUCH_START || e->type == INPUT_EVENT_TYPE_TOUCH_END) {
        Widget *w = processTouch(view_manager::currentView, e);
        ...
        switch (e->type) {
            ...
            case INPUT_EVENT_TYPE_ENCODER:
                ...
                if (menuStatus != ACTIVE) {
                    if (RotBtnInputPin.getState() ==
                        GPIO_PIN_RESET) { // with push button low, change the step size instead of frequency
