...
namespace cat_protocol {
    ...
    static usb_request_handler_fn request_handlers[] = {
            NULL,
            NULL,
            NULL,
...
//    }
    ...
    uint64_t parse_freq(uint8_t *data) {
        ...
        uint64_t freq = (data[0] & 0x0F) + (((data[0] >> 4) & 0x0F) * 10) +
                        ((data[1] & 0x0F) * 100) + (((data[1] >> 4) & 0x0F) * 1000) +
                        ((data[2] & 0x0F) * 10000) + (((data[2] >> 4) & 0x0F) * 100000) +
                        ((data[3] & 0x0F) * 1000000) + (((data[3] >> 4) & 0x0F) * 10000000) +
