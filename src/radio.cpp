...
namespace radio {
    ...
    const st_band bands[] = {
            {420000000, 450000000, FLT_4_CODE, LOW_SIDE,  true},
    ...
    void calculate_freqs() {
        ...
        switch (config.modulation) {
            ...
            case SSB_USB:
                ...
                mixers[1].setLoInjection(config.modulation == SSB_USB
                                         ? (LO_INJECTION) (mixers[0].getLoInjection() * -1)
                                         : mixers[0].getLoInjection());
                mixers[1].setRf(config.f_1st_if);

                // Apply an offset to put the left sideband onto the filter passband
                offset = (int) (if_bw_khz * 1000 / 2) +
                         (is_freq_inverted() ? 1000 : 500); // +500 to account for the skirt
            ...
            case AM:
                ...
                mixers[0].setLoInjection(band.lo_injection == ANY_SIDE
                                         ? config.lo_injection
    ...
    void change_step(int amount) {
        ...
        if ((mostSignificantDecimal(config.f_step)) != 1) {

            // changing step in tune mode should be in powers of 10
            config.f_step = 1000;
        } else {

            if (amount > 0) config.f_step *= 10 * amount;
            else config.f_step /= 10 * -amount;

            if (config.f_step > 1000000UL) {
                config.f_step = 10;
            } else if (config.f_step <= 0) {
                config.f_step = 1000000UL;
            }
        }
    }

    void change_frequency(int amount) {
        set_frequency(config.f_carrier + amount * config.f_step);
        scanner::stop();
    }
    ...
    void update_freq() {
        config.f_carrier = constrain(config.f_carrier, config.f_min, config.f_max);
        calculate_freqs();

        st_freq_event event{config.f_carrier, BEFORE_UPDATE};
        ...
        event = {config.f_carrier, AFTER_UPDATE};
        freq_signal.emit(&event);
    }
    ...
    uint64_t get_band() {
        return find_band(config.f_carrier);
    }
    ...
    void task_loop() {
        ...
        if (config.f_carrier != radio::f_last) {
            ...
            if (config.band == radio::BAND_AUTO) {
                ...
                if (config.f_carrier > radio::bands[band].freq_end) {
                    ...
                    if (band > 0) {
                        ...
                        //}
                        config.f_carrier = radio::bands[band - 1].freq_start;
                    } else {
                        config.f_carrier = radio::bands[band].freq_end;
                    }

                } else if (config.f_carrier < radio::bands[band].freq_start) {

                    if (band < (radio::BAND_AUTO - 1)) {
                        config.f_carrier = radio::bands[band + 1].freq_end;
                    } else {
                        config.f_carrier = radio::bands[band].freq_start;
                    }
                }
            }
