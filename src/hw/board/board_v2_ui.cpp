...
namespace boardUI {
    ...
    prompt *CMX973_VGB_Gains[] = {
            new Menu::menuValue<IF_GAIN>(IFGainNames[IF_GAIN_0], IF_GAIN_0, changeCMX973Gain, focusEvent),
    ...
    Menu::select<IF_GAIN> &CMX973VGAGainMenu =
            *new Menu::select<IF_GAIN>("VGA Gain",
                                       config.hw.cmx973_vga,
                                       sizeof(CMX973_VGA_Gains) / sizeof(prompt *),
                                       CMX973_VGA_Gains, doNothing, updateEvent);

    Menu::select<IF_GAIN> &CMX973VGBGainMenu =
            *new Menu::select<IF_GAIN>("VGB Gain",
                                       config.hw.cmx973_vgb,
                                       sizeof(CMX973_VGB_Gains) / sizeof(prompt *),
                                       CMX973_VGB_Gains, doNothing, updateEvent);
    ...
    MENU(boardMenu, "Hardware", doNothing, noEvent, noStyle,
         SUBMENU(CMX973VGAGainMenu),
         SUBMENU(CMX973VGBGainMenu),
