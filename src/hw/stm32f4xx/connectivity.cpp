...
void setup_connectivity() {
    ...
    /* I2C */
    // MX_I2C1_Init(); // This is the available SPI at the pin header. We don't use it unless needed
    MX_I2C2_Init();
 ...
 * This is required if the DMA FIFO is enabled and we are transferring to the peripheral.
 * Otherwise, the transfer complete callback handler won't be fired, since
 * it is fired from HAL_SPI_IRQHandler only when the FIFO is empty
 */
