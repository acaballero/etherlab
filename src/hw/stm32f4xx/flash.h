...
/* Start address of the settings in flash memory space.
 * On STM32f427, the flash starts at 0x080000000, has (with single bank enabled))
...
//#define DATA_EEPROM_START_ADDR (0x080C0000)
//#define DATA_EEPROM_SIZE_BYTES 131072

uint8_t flash_write(uint16_t *src, uint32_t size);
uint8_t flash_read(uint16_t *src, uint32_t size);
