...
Menu::select<MODULATION_MODE> &modulationMenu =
        *new Menu::select<MODULATION_MODE>("Modulation",
                                           config.modulation,
                                           sizeof(modulationValues) / sizeof(prompt *),
...
prompt *bandValues[] = {
        new Menu::menuValue<radio::BAND>(radio::bandNames[radio::BAND_AUTO], radio::BAND_AUTO),
...
const char *colorNames[] = {"Black", "Grey darker", "Grey dark",
                            "Grey ligh", "White", "Navy",
                            "Green dark", "Cyan dark", "Maroon",
                            "Olive", "Blue", "Green",
                            "Red", "Magenta", "Yellow",
...
prompt *colorValues[23] = {
        new Menu::menuValue<uint16_t>(colorNames[0], C565_BLACK),
        new Menu::menuValue<uint16_t>(colorNames[1], C565_GREY_DARKER),
        new Menu::menuValue<uint16_t>(colorNames[2], C565_GREY_DARK),
        new Menu::menuValue<uint16_t>(colorNames[3], C565_GREY_LIGHT),
        new Menu::menuValue<uint16_t>(colorNames[4], C565_WHITE),
        new Menu::menuValue<uint16_t>(colorNames[5], C565_NAVY),
        new Menu::menuValue<uint16_t>(colorNames[6], C565_GREEN_DARK),
        new Menu::menuValue<uint16_t>(colorNames[7], C565_CYAN_DARK),
        new Menu::menuValue<uint16_t>(colorNames[8], C565_MAROON),
        new Menu::menuValue<uint16_t>(colorNames[9], C565_OLIVE),
        new Menu::menuValue<uint16_t>(colorNames[10], C565_BLUE),
        new Menu::menuValue<uint16_t>(colorNames[11], C565_GREEN),
        new Menu::menuValue<uint16_t>(colorNames[12], C565_RED),
        new Menu::menuValue<uint16_t>(colorNames[13], C565_MAGENTA),
        new Menu::menuValue<uint16_t>(colorNames[14], C565_YELLOW),
        new Menu::menuValue<uint16_t>(colorNames[15], C565_ORANGE),
        new Menu::menuValue<uint16_t>(colorNames[16], C565_GREENYELLOW),
        new Menu::menuValue<uint16_t>(colorNames[17], C565_PINK),
        new Menu::menuValue<uint16_t>(colorNames[18], C565_BROWN),
        new Menu::menuValue<uint16_t>(colorNames[19], C565_GOLD),
        new Menu::menuValue<uint16_t>(colorNames[20], C565_SILVER),
        new Menu::menuValue<uint16_t>(colorNames[21], C565_SKYBLUE),
...
Menu::select<radio::BAND> &bandMenu =
        *new Menu::select<radio::BAND>("Band",
                                       config.band,
                                       sizeof(bandValues) / sizeof(prompt *),
                                       bandValues, changeBand, exitEvent);
...
Menu::select<uint8_t> &filterMenu =
        *new Menu::select<uint8_t>("Frontend filter",
                                   config.filter,
                                   sizeof(bandValues) / sizeof(prompt *),
                                   bandValues, changeFilter, exitEvent);
...
Menu::select<radio::IF_FILTER> &IFFilterMenu =
        *new Menu::select<radio::IF_FILTER>("IF Filter",
                                            config.if_filter,
                                            sizeof(IFFilterValues) / sizeof(prompt *),
                                            IFFilterValues, changeIFFilter, exitEvent);
...
Menu::select<radio::RPT_MODE> &repeaterMenu =
        *new Menu::select<radio::RPT_MODE>("Repeater mode",
                                           config.repeater_mode,
                                           sizeof(repeaterValues) / sizeof(prompt *),
                                           repeaterValues, changeRepeater, exitEvent);

MENU(menuTune, "Tune", doNothing, anyEvent, noStyle,
     SUBMENU(modulationMenu),
     SUBMENU(bandMenu),
     SUBMENU(filterMenu),
...
prompt *driveStrengthValues[] = {
        new Menu::menuValue<LO_POWER>("Low (-4 dBm)", LO_POWER_LOW),
        new Menu::menuValue<LO_POWER>("Medium (0 dBm)", LO_POWER_MEDIUM),
        new Menu::menuValue<LO_POWER>("High (4 dBm)", LO_POWER_HIGH)

};

prompt *frontendPathValues[] = {
        new Menu::menuValue<radio::FRONTEND_PATH>("Attenuator (-10 dB)", radio::FRONTEND_PATH_ATT),
...
prompt *loInjectionValues[] = {
        new Menu::menuValue<LO_INJECTION>("LO", LOW_SIDE),
        new Menu::menuValue<LO_INJECTION>("HIGH", HIGH_SIDE)
};
...
Menu::select<LO_INJECTION> &loSideInjectionMenu =
        *new Menu::select<LO_INJECTION>("Preferred LO inj. side",
                                        config.lo_injection,
                                        sizeof(loInjectionValues) / sizeof(prompt *),
                                        loInjectionValues);
...
Menu::select<LO_POWER> &driveStrength1stLOMenu =
        *new Menu::select<LO_POWER>("1st LO drive",
                                    config.lo_drive_strength_0,
                                    sizeof(driveStrengthValues) / sizeof(prompt *),
                                    driveStrengthValues, changeDriveStrength, exitEvent);

Menu::select<LO_POWER> &driveStrength2ndLOMenu =
        *new Menu::select<LO_POWER>("2nd LO drive",
                                    config.lo_drive_strength_1,
                                    sizeof(driveStrengthValues) / sizeof(prompt *),
                                    driveStrengthValues, changeDriveStrength, exitEvent);
...
PADMENU(dateMenu, "Date", setDate, updateEvent, noStyle,
        FIELD(date.Year, "", "/", 22, 99, 1, 0, setDate, exitEvent, noStyle),
        FIELD(date.Month, "", "/", 1, 12, 1, 0, setDate, exitEvent, wrapStyle),
        FIELD(date.Date, "", "", 1, 31, 1, 0, setDate, exitEvent, wrapStyle));

PADMENU(timeMenu, "Time", setTime, updateEvent, noStyle,
        FIELD(time.Hours, "", ":", 0, 23, 1, 0, setTime, exitEvent, wrapStyle),
        FIELD(time.Minutes, "", "", 1, 59, 1, 0, setTime, exitEvent, wrapStyle));

#endif

MENU(menuSettings, "Settings", doNothing, anyEvent, noStyle,
     SUBMENU(debugToggleMenu),
     SUBMENU(enableHPAToggleMenu),
     FIELD(config.max_power_dbm, "HPA power limit", "dBm.", 0, 50, 1, 0, doNothing, exitEvent,
           wrapStyle),
     SUBMENU(frontendPathMenu),
     FIELD(config.coupler_0db_mv, "Coupler 0 dB offset", "mV.", 0, 5000, 5, 0, changeCouplerOffset, exitEvent,
           wrapStyle),
     SUBMENU(driveStrength1stLOMenu),
     SUBMENU(driveStrength2ndLOMenu),
     SUBMENU(loSideInjectionMenu),
     altFIELD(engPlaces<3>::menuField, config.f_1st_if, "1st. IF Frequency", "kHz.", 0, 100000000, 1000, 10000,
              changeCalibration, exitEvent, noStyle),
     altFIELD(engPlaces<3>::menuField, config.f_if_fm_tx, "FM IF TX Frequency", "kHz.", 0, 100000000, 1000, 10000,
              changeCalibration, exitEvent, noStyle),
     FIELD(config.f_correction, "LO Ref. Correction", "Hz.", -1000000, 1000000, 10, 1, changeCalibration, exitEvent,
           wrapStyle),
     FIELD(config.if_correction, "IF Correction", "Hz.", -1000000, 1000000, 10, 0, changeCalibration, exitEvent,
           wrapStyle),
     #if ENABLE_RTC
             SUBMENU(dateMenu),
     SUBMENU(timeMenu),
...
MENU(menuDSP, "DSP", doNothing, anyEvent, noStyle,
     SUBMENU(dspCaptureUI::captureMenu),
     SUBMENU(dspReplayUI::replayMenu),
...
Menu::select<MODULATION_MODE> &freqMemModulationMenu =
        *new Menu::select<MODULATION_MODE>("Modulation",
                                           tempFreqMem.mode,
                                           sizeof(modulationValues) / sizeof(prompt *),
                                           modulationValues, updateRadio, exitEvent);
...
MENU(freqMemEditMenu, "Frequency edit", doNothing, noEvent, wrapStyle,
     EDIT("Name", tempFreqMem.name, alphaNumMask, doNothing, noEvent, noStyle),
     SUBMENU(freqMemModulationMenu),
     EDIT("Frequency (khz)", tempFreqBuf, digitMask, doNothing, noEvent, noStyle),
    ...
    Used printItem(menuOut &out, int idx, int len) override {
        ...
        if (len) {
            ...
            if (empty) {
                ...
                snprintf(buf, 35, "[%2d] %-4s %12s  %*s", idx, radio::modulationNames[fm.mode], sf, FREQ_MEM_NAME_SIZE,
                         fm.name);
            }
...
FreqMemoryMenu freqMemMenu("Frequency memory", FREQ_MEM_SIZE, "<Back", freqMemEditMenu, freqMemorySelectedEvent,
                           enterEvent);
...
MENU(mainMenu, "Main menu", doNothing, noEvent, noStyle,
     SUBMENU(menuTune),
             ...
             SUBMENU(menuDSP),
     #endif
             SUBMENU(scanner_ui::menuScan),
     SUBMENU(fftUI::fftMenu),
     SUBMENU(menuSettings),
     SUBMENU(boardUI::boardMenu),
     OBJ(freqMemMenu),
     EXIT("<Back")
);
...
const colorDef<uint16_t> menuColors[8] MEMMODE = {
        ...
        {{C565_GREY_LIGHT,  C565_GREY_LIGHT},  {C565_WHITE,       C565_WHITE,       C565_BLACK}},// fgColor
        ...
        {{C565_TRANSPARENT, C565_BLACK},       {C565_TRANSPARENT, C565_GREY_LIGHT,  C565_WHITE}},// cursorColor
        ...
        {{C565_TRANSPARENT, C565_GREY_DARK},   {C565_TRANSPARENT, C565_GREY_LIGHT,  C565_BLUE}}// selectColor
};
