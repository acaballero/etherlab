...
void FrequencyWidget::paint_callback() {
    ...
    if (config.f_step > 100) start_line -= 6; // sip hundreds separator
    if (config.f_step > 100000) start_line -= 6; // skip thousands separator
...
void FrequencyWidget::do_paint() {
    ...
    st_freqInfo freqInfo = {
            (unsigned long) radio::get_frequency(),
            config.f_step,
