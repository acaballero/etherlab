...
adc_type dsp_temp_buf_data[DSP_BLOCK * 2]; // complex_t is twice the size of adc_type (I/Q samples), so a temp buffer to store only one half of the adc_buff is DSP_BLOCK*2 adc_type items
...
uint8_t dsp_fifo_buff[DSP_FIFO_SIZE];

FIFO fifo((char *) dsp_fifo_buff, DSP_FIFO_SIZE);
