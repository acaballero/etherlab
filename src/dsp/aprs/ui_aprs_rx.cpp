...
template <> void RecentEntriesTable<APRSRecentEntries>::draw(const Entry &entry, const Rect &target_rect, Painter &painter, const Style &style) {
    Color target_color;
...
APRSRxView::APRSRxView(NavigationView &nav, Rect parent_rect) : View(parent_rect), nav_{nav} {
    baseband::run_image(portapack::spi_flash::image_tag_aprs_rx);

    add_children({&rssi, &channel, &field_rf_amp, &field_lna, &field_vga, &field_volume, &options_region, &field_frequency, &record_view, &console});
...
void APRSRxView::on_packet(const APRSPacketMessage *message) {
    ...
    // if(reset_console){ //having more than one console causes issues when switching tabs where one is disabled, and the other enabled breaking the scoll
    // setup.
    //	console.on_hide();
    //	console.on_show();
    //	reset_console = false;
...
APRSDetailsView::APRSDetailsView(NavigationView &nav) {
    ...
    button_see_map.on_select = [this, &nav](Button &) {
        geomap_view = nav.push<GeoMapView>(entry_copy.source_formatted,
                                           0, // entry_copy.pos.altitude,
                                           GeoPos::alt_unit::FEET, GeoPos::spd_unit::HIDDEN, entry_copy.pos.latitude, entry_copy.pos.longitude,
