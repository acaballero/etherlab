...
class AX25Frame {
  public:
    // 14-byte address field with source and destination (no digipeater path)
    size_t build(char *const address, const uint8_t control, protocol_id_t protocol, const std::string &info, uint16_t *buffer);
    // Variable-length address field (len must be multiple of 7)
    size_t build(char *const address, size_t address_len, uint8_t control, protocol_id_t pid, const std::string &info, uint16_t *buffer);
