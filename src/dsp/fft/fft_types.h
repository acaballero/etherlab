...
enum FFT_SPECTRUM_STYLE {
   FFT_SPECTRUM_STYLE_FILL,
   FFT_SPECTRUM_STYLE_LINE,
   FFT_SPECTRUM_STYLE_LINE_FILL
};
...
#define FFT_FIFO_SIZE_MIN                                                      \
   ((FFT_N + FFT_LPF_FIR_FILTER_DELAY_BLOCKS * DSP_BLOCK) *                    \
    MAX_DECIMATION_FACTOR)
#define FFT_FIFO_SIZE                                                          \
   (((FFT_N + FFT_LPF_FIR_FILTER_DELAY_BLOCKS * DSP_BLOCK) *                   \
     MAX_DECIMATION_FACTOR))
...
typedef struct {
   ...
   uint32_t bw =
       FFT_BANDWIDTH; // Bandwidth of interest of the FFT. Usable bandwidth.
   int16_t min_db = -130;
   int16_t max_db = -30;
   ...
   int maxAmpl =
       (2 / 3.3) * MAX_ADC_VALUE; // 2V peak to peak considering inputs to
