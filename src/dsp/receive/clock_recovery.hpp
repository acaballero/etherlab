...
class LinearErrorFilter {
  public:
    LinearErrorFilter(const float filter_alpha = 0.95f, const float error_weight = -1.0f) : filter_alpha{filter_alpha}, error_weight{error_weight} {
    }
...
template <typename ErrorFilter> class ClockRecovery {
  public:
    ...
    ClockRecovery(const float sampling_rate, const float symbol_rate, ErrorFilter error_filter, SymbolHandler symbol_handler)
