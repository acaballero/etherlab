...
namespace dspCaptureUI {
    ...
    Menu::result on_file_updated(Menu::eventMask e) {
        ((CaptureTask *) tasks[DSP_TASK_CAPTURE])->setFile(FileFactory::getFile(ftype, fname));
    ...
    */
    void get_file_name(char *buff) {
        ...
        if (!filename_is_edited) {
            ...
            sprintf(buff, WAVEFILE_DEFAULT_FILENAME, fft_params.sample_freq / fft_params.decimation_factor,
    ...
    Menu::result on_menu_event(Menu::eventMask e) {
        ...
        CaptureTask *task = ((CaptureTask *) tasks[DSP_TASK_CAPTURE]);

        switch (e) {
            case Menu::enterEvent:

                capture_w.setProcessorStatus(&((DspCaptureProcessor *) processors[DSP_PROCESSOR_CAPTURE])->status);
                capture_w.setTaskStatus(&((CaptureTask *) tasks[DSP_TASK_CAPTURE])->status);
    ...
    Menu::result change_dsp_status(Menu::eventMask e) {
        if (e == Menu::activateEvent) {
            ...
            dsp_command({(DSP_COMMAND) nextCommand, DSP_TASK_CAPTURE}, on_event);
        }
        return Menu::proceed;
    }
    ...
    result change_file_type(eventMask e) {
        get_file_name(fname);
    ...
    prompt *fileTypeValues[] = {
            new Menu::menuValue<FileType>(file_type_names[FTYPE_WAV], FTYPE_WAV),
            new Menu::menuValue<FileType>(file_type_names[FTYPE_CS16], FTYPE_CS16)
    };

    Menu::select<FileType> &fTypeMenu =
            *new Menu::select<FileType>("File type",
                                        ftype,
                                        sizeof(fileTypeValues) / sizeof(prompt *),
                                        fileTypeValues, change_file_type, exitEvent);

    TOGGLE(command, captureToggle, "Command: ", change_dsp_status, Menu::anyEvent, Menu::noStyle,
           VALUE("Stop", DSP_COMMAND_STOP, change_dsp_status, Menu::anyEvent),
           VALUE("Start", DSP_COMMAND_START, change_dsp_status, Menu::anyEvent)
    )

    MENU(captureMenu, "Capture", on_menu_event, (Menu::eventMask) (Menu::enterEvent | Menu::exitEvent), Menu::noStyle,
         SUBMENU(captureToggle),
         EDIT("File:", fname, alphaNumMask, on_file_updated, Menu::updateEvent, Menu::noStyle),
         OBJ(freqEdit),
