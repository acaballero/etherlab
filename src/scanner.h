...
enum SCANNER_STATUS { SCANNER_STATUS_STOPPED, SCANNER_STATUS_RUNNING };

enum SCANNER_MODE { SCANNER_MODE_CUSTOM, SCANNER_MODE_BAND, SCANNER_MODE_LIST };
...
struct st_scanner_info {
    ...
    bool operator==(const st_scanner_info &st) const {
        return freq_min == st.freq_min && freq_max == st.freq_max && freq_step == st.freq_step && status == st.status && squelch == st.squelch &&
               pause_ms == st.pause_ms && period_s == st.period_s && direction == st.direction && save_found == st.save_found;
    }

    bool operator!=(const st_scanner_info &st) const { return !(*this == st); }
};
