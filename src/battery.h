...
// A voltage divider is used to bring the input voltage to an appropriate level for the ADC's input range
...
namespace battery {
    ...
    enum BATTERY_STATUS {
        BATTERY_STATUS_UNDEFINED, BATTERY_STATUS_LOW, BATTERY_STATUS_MEDIUM, BATTERY_STATUS_HIGH, BATTERY_STATUS_CHARGING
    };
    ...
    struct battery_st_info {
        ...
        bool operator==(const battery_st_info &st) const {
            return voltage == st.voltage && capacity == st.capacity;
        }
    };
