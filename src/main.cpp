 ...
 * a logarithmc amplifier and limiter for the FM/AM detector, which has an RSSI output.
 * The AF signal strenght cannot be used for FM, as the detector generates high noise power
 * when the RF signal is below the detection threshold. Also, in SSB mode, the log amp of the
 * AM/FM detector board is disabled. Therefore, until I develop an RSSI level
 * measurement circuit (using the IF signal) which is available for all modulations, the signal
 * strenght must be calculated using either the AF signal strenght board, for SSB, or the RSSI level
 * of the log amp, for AM/FM
 */
...
void view_loop() {
    // TODO: Delegate dirty state management to the widget itself based on information change messages and refresh rate
...
/*
 * Enables sending printf / puts strings through the SWO pin when a debugger is attached
 * If a debugger is present, it should enable ITM on the target, otherwise ITM_Sendchar will return without doing anything
 */
