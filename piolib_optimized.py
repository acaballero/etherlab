...
class LibBuilderBase:
    ...
    def get_implicit_includes(  # pylint: disable=too-many-branches
        ...
        #click.echo(f"get_implicit_includes: start")
    ...
    def search_deps_recursive(self, search_files=None):
        ...
        #click.echo(f"!!!!!!!!!!!!!!!!! {datetime.datetime.now()} About to search in " + str(search_files))

        lib_inc_map = {}
        incs = self.get_implicit_includes(search_files)

        for inc in incs:
            inc_path = inc.get_abspath()
            #click.echo(f"{datetime.datetime.now()} Implicit include: " + str(inc_path))
            for lb in self.env.GetLibBuilders():

                #click.echo(f"{datetime.datetime.now()} LibBuilders: " + str(lb))
                
                if inc_path in lb:
                    #click.echo(f"{lb.path} Found!!!")
...
def ConfigureProjectLibBuilder(env):
    ...
    click.echo("Project" + repr(project))
