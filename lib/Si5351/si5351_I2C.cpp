...
Si5351::Si5351(uint8_t i2c_addr) : i2c_bus_addr(i2c_addr << 1) { // The address must be shifted 1 position to the left in 7-bit address mode
 ...
 *
 */
bool Si5351::init(I2C_HandleTypeDef i2chandler, uint8_t xtal_load_c, uint32_t xo_freq, int32_t corr) {
    ...
    uint8_t addr = SI5351_DEVICE_STATUS;

    this->hi2c1 = i2chandler;

    if (HAL_I2C_Master_Transmit(&hi2c1, i2c_bus_addr, &addr, 1, 50) == HAL_OK) {
        // Wait for SYS_INIT flag to be clear, indicating that device is ready
        do {
            HAL_I2C_Master_Receive(&hi2c1, i2c_bus_addr, &status_reg, 1, 50);
 ...
 */
HAL_StatusTypeDef Si5351::set_freq(uint64_t freq, enum si5351_clock clk) {
    ...
    if ((uint8_t)clk <= (uint8_t)SI5351_CLK5) {
        ...
        uint32_t freq_l = (int32_t)(freq / SI5351_FREQ_MULT);
        uint64_t max_f = (uint64_t)SI5351_MULTISYNTH_SHARE_MAX * (uint64_t)SI5351_FREQ_MULT;

        // Lower bounds check
        if (freq_l > 0 && freq_l < SI5351_CLKOUT_MIN_FREQ) {
            freq_l = SI5351_CLKOUT_MIN_FREQ;
        }

        // Upper bounds check
        if (freq_l > SI5351_MULTISYNTH_MAX_FREQ) {
            freq_l = (int32_t)SI5351_MULTISYNTH_MAX_FREQ;
        }
        ...
        // on same PLL
        bool reset = !clk_first_set[(uint8_t)clk];
        uint32_t current_freq = (int32_t)(clk_freq[(uint8_t)clk] / SI5351_FREQ_MULT);

        if (freq_l > (SI5351_MULTISYNTH_SHARE_MAX)) {
            // Check other clocks on same PLL
            uint8_t i;
            for (i = 0; i < 6; i++) {
                if (clk_freq[i] > max_f) {
                    if (i != (uint8_t)clk && pll_assignment[i] == pll_assignment[clk]) {
                        return HAL_ERROR; // won't set if any other clks already >100 MHz
                    }
                }
            }

            uint32_t boundary_freq = (uint32_t)SI5351_MULTISYNTH_DIVBY4_FREQ;

            // Enable the output on first update_freq only
            if (reset) {
                r = output_enable(clk, 1);
                clk_first_set[(uint8_t)clk] = true;
            }

            if (r == HAL_OK) {
                if (((current_freq >= boundary_freq) && (freq_l < boundary_freq)) || ((current_freq < boundary_freq) && (freq_l >= boundary_freq))) {

                    // the chage in frequency will pass the boundary where fractional division is needed, so we need to reset the PLL
                    reset = true;
                } else if (abs((int32_t)current_freq - (int32_t)freq_l) > 10000000) {

                    /*
                    Serial.println(abs(current_freq - freq_l));
                    Serial.println(current_freq);
                    Serial.println(freq_l);
                    Serial.println();
                     */

                    // Reset PLL if the change if frequency is big
                    reset = true;
                }
                ...
                // Set PLL
                r = set_pll(pll_freq, pll_assignment[clk]);
                ...
                for (i = 0; i < 6 && !r; i++) {
                    if (clk_freq[i] != 0) {
                        if (pll_assignment[i] == pll_assignment[clk]) {
                            ...
                            r = set_ms((enum si5351_clock)i, temp_reg, int_mode, r_div, div_by_4);
                        }
                    }
                }
            }
            ...
            if (abs((int32_t)current_freq - (int32_t)freq_l) > 10000000) {
                reset = true;
            }
            ...
            // Set multisynth registers
            r = set_ms(clk, ms_reg, int_mode, r_div, div_by_4);
        }

        // Reset the PLL
        if (reset && !r) {
            r = pll_reset(pll_assignment[clk]);
        }
        ...
        if (clk == SI5351_CLK6) {
            if (clk_freq[7] != 0) {
                ...
                // pllb_freq = pll_freq;
                r = set_pll(pll_freq, SI5351_PLLB);
            }
        } else {
            if (clk_freq[6] != 0) {
                if (pllb_freq % freq == 0) {
 ...
 *   (use the si5351_clock enum)
 */
uint8_t Si5351::set_freq_manual(uint64_t freq, uint64_t pll_freq, enum si5351_clock clk, bool vcxo) {
    struct Si5351RegSet ms_reg;
    uint8_t int_mode = 0;
    uint8_t div_by_4 = 0;
    ...
    if (!r) {
        ...
        Si5351RegSet last_ms = clk_ms[(int8_t)clk];
        if (last_ms.p1 != ms_reg.p1 || last_ms.p2 != ms_reg.p2 || last_ms.p3 != ms_reg.p3) {
            // Set multisynth registers (MS must be set before PLL)
            set_ms(clk, ms_reg, int_mode, r_div, div_by_4);
            clk_ms[(int8_t)clk] = ms_reg;
        }
    }
 ...
 */
HAL_StatusTypeDef Si5351::set_pll(uint64_t pll_freq, enum si5351_pll target_pll) {
    ...
    if (target_pll == SI5351_PLLA) {
        ret = si5351_write_bulk(SI5351_PLLA_PARAMETERS, i, params);
        plla_freq = pll_freq;
    } else if (target_pll == SI5351_PLLB) {
        ret = si5351_write_bulk(SI5351_PLLB_PARAMETERS, i, params);
        pllb_freq = pll_freq;
    }
 ...
 *   Set to 1 to enable, 0 to disable
 */
HAL_StatusTypeDef Si5351::set_ms(enum si5351_clock clk, struct Si5351RegSet ms_reg, uint8_t int_mode, uint8_t r_div, uint8_t div_by_4) {
    uint8_t params[20];
    uint8_t i = 0;
    uint8_t temp;
    uint8_t reg_val;
    ...
    if ((uint8_t)clk <= (uint8_t)SI5351_CLK5) {
        ...
        ret = si5351_read((SI5351_CLK0_PARAMETERS + 2) + (clk * 8), &reg_val);
 ...
 */
HAL_StatusTypeDef Si5351::output_enable(enum si5351_clock clk, uint8_t enable) {

    uint8_t reg_val = 0;
    HAL_StatusTypeDef ret = HAL_OK;

    if (output_enabled[(uint8_t)clk] != enable || !clk_first_set[(uint8_t)clk]) {

        ret = si5351_read(SI5351_OUTPUT_ENABLE_CTRL, &reg_val);
        ...
        if (ret == HAL_OK) {
            ...
            ret = si5351_write(SI5351_OUTPUT_ENABLE_CTRL, reg_val);

            if (ret == HAL_OK) {

                output_enabled[(uint8_t)clk] = enable;
                clk_freq[(uint8_t)clk] = 0;

                ret = output_clk_power(clk, enable);
            }
        }
    }
...
HAL_StatusTypeDef Si5351::output_clk_power(enum si5351_clock clk, uint8_t enable) {

    uint8_t ret = 0;

    si5351_read(SI5351_CLK0_CTRL + (uint8_t)clk, &ret);

    if (enable == 1) {
        ret &= ~SI5351_CLK_POWERDOWN;
    } else {
        ret |= SI5351_CLK_POWERDOWN;
    }

    return si5351_write(SI5351_CLK0_CTRL + (uint8_t)clk, ret);
}

uint8_t Si5351::sleep() {
    uint8_t ret = 0;
    // ret = (HAL_StatusTypeDef) si5351_write(SI5351_OUTPUT_ENABLE_CTRL, 0xFF); // Is not necessary to disable the output
    for (int i = 0; i < 8 && ret == 0; i++) {
        output_clk_power(static_cast<si5351_clock>(i), false);
    }

    return ret;
}

uint8_t Si5351::wakeup() {
    uint8_t ret = 0;
    // uint8_t reg_val = si5351_read(SI5351_OUTPUT_ENABLE_CTRL); // Not necessary to disable the output

    for (int i = 0; i < 8 && ret == 0; i++) {
        output_clk_power(static_cast<si5351_clock>(i), true);
        //        if (output_enabled[ i]) {
        //            reg_val &= ~(1 << i);
        //        }
    }

    // ret = ret & (HAL_StatusTypeDef)si5351_write(SI5351_OUTPUT_ENABLE_CTRL, reg_val);
 ...
 */
HAL_StatusTypeDef Si5351::pll_reset(enum si5351_pll target_pll) {

    HAL_StatusTypeDef r = HAL_ERROR;

    if (target_pll == SI5351_PLLA) {
        r = si5351_write(SI5351_PLL_RESET, SI5351_PLL_RESET_A);
    } else if (target_pll == SI5351_PLLB) {
        r = si5351_write(SI5351_PLL_RESET, SI5351_PLL_RESET_B);
    }
    return r;
}
 ...
 */
HAL_StatusTypeDef Si5351::set_pll_input(enum si5351_pll pll, enum si5351_pll_input input) {
    uint8_t reg_val;
    HAL_StatusTypeDef ret = si5351_read(SI5351_PLL_INPUT_SOURCE, &reg_val);
    ...
    if (ret == HAL_OK) {
        ...
        ret = si5351_write(SI5351_PLL_INPUT_SOURCE, reg_val);
 ...
 */
uint8_t Si5351::set_vcxo(uint64_t pll_freq, uint8_t ppm) {
    struct Si5351RegSet pll_reg;
    uint64_t vcxo_param;
    ...
    uint8_t ret = si5351_write(SI5351_VXCO_PARAMETERS_LOW, temp);

    temp = (uint8_t)((vcxo_param >> 8) & 0xFF);
    ret = si5351_write(SI5351_VXCO_PARAMETERS_MID, temp);

    temp = (uint8_t)((vcxo_param >> 16) & 0x3F);
    ret = si5351_write(SI5351_VXCO_PARAMETERS_HIGH, temp);
...
HAL_StatusTypeDef Si5351::si5351_write_bulk(uint8_t addr, uint8_t bytes, uint8_t *data) {

    /*
    i2c_start((I2C_7BITADDR << 1) | I2C_WRITE);
    bool b = i2c_write(addr);
    ...
    HAL_StatusTypeDef ret = HAL_I2C_Mem_Write(&hi2c1, i2c_bus_addr, addr, I2C_MEMADD_SIZE_8BIT, data, bytes, 20);
...
HAL_StatusTypeDef Si5351::si5351_write(uint8_t addr, uint8_t data) {
    ...
    i2c_stop();
     */
    HAL_StatusTypeDef ret = HAL_I2C_Mem_Write(&hi2c1, i2c_bus_addr, addr, I2C_MEMADD_SIZE_8BIT, &data, 1, 20);

    return ret;
}

HAL_StatusTypeDef Si5351::si5351_read(uint8_t addr, uint8_t *reg_val) {
    ...
    HAL_StatusTypeDef ret =
        HAL_I2C_Master_Transmit(&hi2c1, i2c_bus_addr, &addr, 1, 20); // note the & operator which gives us the address of the register_pointer variable

    if (ret == HAL_OK) {
        // receive the 2 x 8bit data into the receive buffer
        ret = HAL_I2C_Master_Receive(&hi2c1, i2c_bus_addr, reg_val, 1, 20);
    }

    return ret;
}

uint8_t Si5351::write_regs(si5351b_revb_register_t *regs, uint8_t nregs) {
    uint8_t ret = 0;

    for (int i = 0; i < nregs && !ret; i++) {
        ret = si5351_write(regs[i].address, regs[i].value);
    }
...
uint64_t Si5351::multisynth_calc(uint64_t freq, uint64_t pll_freq, struct Si5351RegSet *reg) {
    ...
    if (pll_freq == 0) {
        // Find largest integer divider for max2
...
HAL_StatusTypeDef Si5351::ms_div(enum si5351_clock clk, uint8_t r_div, uint8_t div_by_4) {
    uint8_t reg_val = 0;
    uint8_t reg_addr = 0;
    ...
    HAL_StatusTypeDef ret = si5351_read(reg_addr, &reg_val);
    ...
    if (ret == HAL_OK) {
        ...
        ret = si5351_write(reg_addr, reg_val);
    }
