...
GPIO_PinState MCP23017InputPin::read() {
    ...
    GPIO_PinState s = (GPIO_PinState)(this->handle->gpio[this->port] & (1 << pin));
...
void MCP23017InputPin::init() {
    ...
    if (this->mode == PINMODE_IT) {
        ...
        mcp23017_writereg(this->handle, REGISTER_IPOLA | this->port,
                          data | (1 << this->pin)); // invert polarity (buttons are pulled up, so interrupts occur when they go low)
