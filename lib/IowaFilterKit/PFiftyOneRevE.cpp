...
int PFiftyOne(long double *Coeff, int Degree, long double *RealRoot, long double *ImagRoot) {
    ...
    long double P[N + 2], QuadQP[N + 2], RealQP[N + 2], QuadK[N + 2], RealK[N + 2], QK[N + 2];
...
//    }
    ...
    while (N > 4 && TypeOfK < MAX_NUM_K) {
        ...
        for (TypeOfK = 0; TypeOfK < MAX_NUM_K &&
                          NZ == 0; TypeOfK++)                   // Iterate on the different possible QuadK inits.
        {
            for (AngleNumber = 0;
                 AngleNumber < MAX_NUM_ANGLES && NZ == 0; AngleNumber++)  // Iterate on the angle used to init TUV.
            {
                SetTUVandK(P, N, TUV, RealK, QuadK, QuadX, AngleNumber, TypeOfK);     // Init TUV and both K's
                for (Iter = 0; Iter < N && NZ ==
                                           0; Iter++)                                  // Allow N calls to QuadIterate for N*QUAD_ITER_MAX iterations, then try a different angle.
                {
...
int QuadIterate(int P51_Iter, long double *P, long double *QP, long double *K, long double *QK, int N, long double *TUV,
                TUpdateStatus *UpdateStatus) {
    int Iter;
...
void UpdateTUV(int Iter, long double *P, int N, long double *QP, long double *K, long double *QK, long double *TUV,
               TUpdateStatus *UpdateStatus) {
    int j;
...
int RealIterate(int P51_Iter, long double *P, long double *QP, long double *K, long double *QK, int N,
                long double *RealZero) {
    int Iter, k;
...
void SetTUVandK(long double *P, int N, long double *TUV, long double *RealK, long double *QuadK, long double X,
                int AngleNumber, int TypeOfQuadK) {
    int j;
    long double a, Theta;
