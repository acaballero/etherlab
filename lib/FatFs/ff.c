...
#define _EXCVT                                                                                                                                                 \
    {                                                                                                                                                          \
        0x80, 0x9A, 0x90, 0x41, 0x8E, 0x41, 0x8F, 0x80, 0x45, 0x45, 0x45, 0x49, 0x49, 0x49, 0x8E, 0x8F, 0x90, 0x92, 0x92, 0x4F, 0x99, 0x4F, 0x55, 0x55, 0x59,  \
...
static FRESULT find_volume(                    /* FR_OK(0): successful, !=0: any error occurred */
    ...
    *rfs = fs;    /* Return pointer to the file system object */
    ...
    if (fmt == FS_FAT32 /* Enable FSINFO only if FAT32 and BPB_FSInfo is 1 */
        && LD_WORD(fs->win.d8 + BPB_FSInfo) == 1 && move_window(fs, bsect + 1) == FR_OK) {
        fs->fsi_flag = 0;
        if (LD_WORD(fs->win.d8 + BS_55AA) == 0xAA55 /* Load FSINFO data if available */
            && LD_DWORD(fs->win.d8 + FSI_LeadSig) == 0x41615252 && LD_DWORD(fs->win.d8 + FSI_StrucSig) == 0x61417272) {
#if (_FS_NOFSINFO & 1) == 0
