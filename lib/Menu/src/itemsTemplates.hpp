...
#define constrain(amt, low, high) ((amt)<(low)?(low):((amt)>(high)?(high):(amt)))
    ...
    bool menuField<T>::changed(const navNode &nav, const menuOut &out, bool sub, bool test) {
        ...
        trace(if (test && (reflex != target()))
                  MENU_DEBUG_OUT << "reflex!=target reflex:" << reflex << " target:" << target() << endl);
    ...
    Used menuField<T>::printTo(navRoot &root, bool sel, menuOut &out, idx_t idx, idx_t len, idx_t panelNr) {
        trace(print_P(out, getText());
                      MENU_DEBUG_OUT << " menuField<T>::printTo " << reflex << endl);
        reflex = target();
    ...
    void menuField<T>::parseInput(navNode &nav, menuIn &in) {
        ...
        if (neg || strchr(numericChars, nc)) {//a numeric value was entered
            if (in.numValueInput) {
                target() = (T) ((neg ? -1 : 1) *
                                in.parseFloat());//TODO: use template specialization and proper convertion
                tunning = true;
                doNav(nav, enterCmd);
    ...
    template<uint8_t dps>
    template<typename T>
    idx_t engPlaces<dps>::menuField<T>::printReflex(menuOut &o) const {
