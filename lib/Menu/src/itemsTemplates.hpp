...
#ifdef MENU_ASYNC
template <typename T> bool menuValue<T>::async(const char *uri, navRoot &root, idx_t lvl) {
    trace(MENU_DEBUG_OUT << (*(prompt *)this) << " menuValue::async! lvl:" << lvl << " navRoot.level:" << root.level
...
template <typename T> bool menuField<T>::changed(const navNode &nav, const menuOut &out, bool sub, bool test) {
...
template <typename T> void menuField<T>::parseInput(navNode &nav, menuIn &in) {
    ...
    if (nc == '-') {
        in.read();
        volatile char ch = in.peek();
        if (!ch || !strchr(numericChars, ch)) {
            doNav(nav, downCmd);
            return;
        }
...
template <uint8_t dps> template <typename T> idx_t decPlaces<dps>::menuField<T>::printReflex(menuOut &o) const {
    return o.print((float)(menuField<T>::reflex), (int)dps);
}

template <uint8_t dps> template <typename T> idx_t engPlaces<dps>::menuField<T>::printReflex(menuOut &o) const {
    return o.print((long)(menuField<T>::reflex / pow(10, dps)));
}

template <typename T> Used choose<T>::printTo(navRoot &root, bool sel, menuOut &out, idx_t idx, idx_t len, idx_t panelNr) {
    bool ed = this == root.navFocus;
    return ed ? prompt::printTo(root, sel, out, idx, len, panelNr) : menuVariantBase::printTo(root, sel, out, idx, len, panelNr);
}

template <typename T> Used toggle<T>::printTo(navRoot &root, bool sel, menuOut &out, idx_t idx, idx_t len, idx_t panelNr) {
...
template <typename T> result toggle<T>::sysHandler(SYS_FUNC_PARAMS) {
...
template <typename T> idx_t menuVariant<T>::sync(idx_t i) {
#ifdef MENU_DEBUG
    if (!(i >= 0 && i < sz())) {
...
template <typename T> bool menuVariant<T>::changed(const navNode &nav, const menuOut &out, bool sub, bool test) {
...
template <typename T> bool choose<T>::changed(const navNode &nav, const menuOut &out, bool sub, bool test) {
