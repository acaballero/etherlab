...
class prompt {
  ...
  public:
    ...
    virtual Used printTo(navRoot &root, bool sel, menuOut &out, idx_t idx, idx_t len,
                         idx_t panelNr = 0); // raw print to output device
...
class navTarget : public prompt {
  public:
    ...
    inline navTarget(constText *t, action a = doNothing, eventMask e = noEvent, styles s = noStyle, systemStyles ss = _noStyle) : prompt(t, a, e, s, ss) {
    }
...
class textField : public navTarget {
  public:
    ...
    inline textField(constText *label, char *b, idx_t sz, char *const *v, action a = doNothing, eventMask e = noEvent, styles style = noStyle,
...
//--------------------------------------------------------------------------
template <typename T> class menuField : public fieldBase {
  public:
    T reflex;
    ...
    menuField(T &value, constText *text, constText *units, T low, T high, T step, T tune, action a = doNothing, eventMask e = noEvent, styles s = noStyle)
...
template <uint8_t dps> struct decPlaces {
    template <typename T> struct menuField : Menu::menuField<T> {
...
template <uint8_t dps> struct engPlaces {
    template <typename T> struct menuField : Menu::menuField<T> {
        using Menu::menuField<T>::menuField;
...
//--------------------------------------------------------------------------
template <typename T> class menuValue : public prompt {
  public:
    inline menuValue(constMEM menuValueShadow<T> &shadow) : prompt(shadow) {
    }

    inline menuValue(constText *text, T value, action a = doNothing, eventMask e = noEvent) : menuValue(*new menuValueShadow<T>(text, value, a, e)) {
    }

    T target() const {
        menuValueShadow<T> *o = (menuValueShadow<T> *)shadow;
        return o->target();
    }
...
class menuNode : public navTarget {
  public:
    ...
    inline menuNode(const char *text, idx_t sz, prompt *constMEM data[], action a = noAction, eventMask e = noEvent, styles style = wrapStyle,
    ...
    template <bool clear> bool _changes(const navNode &nav, const menuOut &out, bool sub, bool test);
};
...
template <typename T> class menuVariant : public menuVariantBase {
  public:
    idx_t reflex;
...
class select : public menuVariant<T> {
  public:
    ...
    inline select(constText *text, T &target, idx_t sz, prompt *constMEM *data, action a = doNothing, eventMask e = noEvent, styles style = noStyle,
...
class toggle : public menuVariant<T> {
  public:
    ...
    inline toggle(constText *text, T &target, idx_t sz, prompt *constMEM *data, action a = doNothing, eventMask e = noEvent, styles style = noStyle,
...
class choose : public menuVariant<T> {
  public:
    ...
    inline choose(constText *text, T &target, idx_t sz, prompt *constMEM *data, action a = doNothing, eventMask e = noEvent, styles style = noStyle,
