...
#define MENU_OUTPUTS(id, maxDepth, ...)                                                                                                                        \
    ...
    Menu::menuOut *constMEM id##_outPtrs[] MEMMODE = {XFOR_EACH(CALL, WITH(REF, id, maxDepth), __VA_ARGS__)};                                                  \
...
#define PADMENU(id, text, aFn, mask, style, ...)                                                                                                               \
    altMENU(Menu::menu, id, text, aFn, mask, style, (Menu::_asPad | Menu::_menuData | Menu::_canNav | Menu::_parentDraw), __VA_ARGS__)
#define altMENU(objType, id, text, aFn, mask, style, ss, ...)                                                                                                  \
    FOR_EACH(DECL, __VA_ARGS__)                                                                                                                                \
    constText id##_text[] MEMMODE = text;                                                                                                                      \
    Menu::prompt *constMEM id##_data[] MEMMODE = {FOR_EACH(DEF, __VA_ARGS__)};                                                                                 \
    constMEM MEMMODE Menu::menuNodeShadows id##Shadows = {(Menu::callback)aFn,                                                                                 \
...
#define altVARIANT(objType, ss, target, id, text, action, mask, style, ...)                                                                                    \
    ...
    constMEM MEMMODE Menu::menuVariantShadows<typeof(target)> id##Shadows = {                                                                                  \
        (Menu::callback)action, ss, id##_text, mask, style, sizeof(id##_data) / sizeof(prompt *), id##_data, &target};                                         \
...
#define DECL_ITEM_(cnt, objType, text, aFn, mask, ss, ...)                                                                                                     \
    ...
    constMEM MEMMODE Menu::promptShadowRaw opShadowRaw##cnt = {(Menu::callback)aFn, ss, title_##cnt, mask, noStyle};                                           \
...
#define DECL_OP_(cnt, objType, text, aFn, mask)                                                                                                                \
    constText title_##cnt[] MEMMODE = text;                                                                                                                    \
    constMEM MEMMODE Menu::promptShadows opShadows##cnt = {(Menu::callback)aFn, _noStyle, title_##cnt, mask, noStyle};                                         \
...
#define DECL_FIELD_(cnt, objType, ss, target, text, units, low, high, step, tune, action, mask, style)                                                         \
    ...
    constMEM MEMMODE Menu::menuFieldShadows<typeof(target)> fieldShadows##cnt = {                                                                              \
        (Menu::callback)action, ss, fieldLabel##cnt, mask, style, fieldUnit##cnt, &target, low, high, step, tune};                                             \
...
#define DECL_EDIT_(cnt, objType, ss, label, buf, valid, action, mask, style)                                                                                   \
    ...
    constMEM MEMMODE Menu::textFieldShadows textfieldShadows##cnt = {(Menu::callback)action,        ss, textFieldLabel##cnt, mask, style, buf, valid,          \
...
#define _MK_VALUE(target, cnt, text, value, action, mask)                                                                                                      \
    ...
    constMEM MEMMODE Menu::menuValueShadows<typeof(target)> choice##cnt##ShadowRaw = {(Menu::callback)action, Menu::_noStyle, valueLabel##cnt, mask,           \
                                                                                      Menu::noStyle,          value};                                          \
