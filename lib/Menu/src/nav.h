...
// navigation panels (min2 1) describe output dimensions (in characters)
struct panel {
    idx_t x, y, w, h;
...
class panelsList {
  public:
    ...
    constMEM idx_t sz;
    idx_t cur = 0;
    inline panelsList(panel p[], navNode *nodes[], idx_t sz) : panels(p), nodes(nodes), sz(sz) { reset(); }
    void reset(idx_t from = 0);
    inline constMEM panel operator[](idx_t i) {
// assert(i<sz);
#ifdef USING_PGM
        panel tmp;
...
class navRoot {
  public:
    ...
    inline navRoot(menuNode &root, navNode *path, idx_t d, menuIn &in, outputsList &o) : out(o), in(in), path(path), maxDepth(d - 1) {
        useMenu(root);
        initPath(d - 1);
    }
