...
template <typename C> struct colorDef {
    C disabled[2];
    C enabled[4];
};
...
#define SYS_FUNC_PARAMS Menu::eventMask event, Menu::navNode &nav, Menu::prompt &item
#define SYS_FUNC_VALUES event, nav, item
#define FUNC_PARAMS Menu::eventMask event, Menu::navNode &nav, Menu::prompt &item
...
struct menuConfig {
    menuConfig(char ecur = '>', char dcur = '-', const navCodesDef &nc = defaultNavCodes, bool invKeys = true)
        : selectedCursor(ecur), disabledCursor(dcur), navCodes(nc), invertFieldKeys(invKeys) {
    }
...
extern menuConfig defaultOptions;
extern menuConfig *options;
