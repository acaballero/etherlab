...
void
menuOut::rect(idx_t panelNr, idx_t x, idx_t y, idx_t w, idx_t h, colorDefs c, bool selected, status stat, bool edit) {}

void
menuOut::box(idx_t panelNr, idx_t x, idx_t y, idx_t w, idx_t h, colorDefs c, bool selected, status stat, bool edit) {}
...
Used menuOut::printMenu(navNode &nav, idx_t panelNr) {
    ...
    if (!(nav.root->navFocus->has((systemStyles) (_parentDraw | _menuData)))) {
        ...
        if (all || titleChanged) {
            ...
            trace(MENU_DEBUG_OUT << "{x:" << pan.x << " y:" << pan.y << " w:" << pan.w << " h:" << pan.h << "}"
                                 << endl);
        ...
        for (idx_t i = 0; asPad || (i < maxY(panelNr) - st); i++) {
            ...
            if (all || p.changed(nav, *this, false)) {
                ...
                else drawCursor(ist, selected, p.enabled, false, panelNr);//assuming only one character
                //<------ cursorEnd
#ifdef MENU_FMT_WRAPS
