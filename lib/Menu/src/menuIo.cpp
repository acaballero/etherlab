...
#ifdef ESP8266
template <typename T> menuOut &menuOut::operator<<(T o) {
    (*(Stream *)this) << (o);
    return *this;
}
#endif
...
menuOut &cursorOut::fill(int x1, int y1, int x2, int y2, char ch, colorDefs color, bool selected, status stat, bool edit) {
    for (int r = y1; r <= y2; r++) {
        setCursor(x1, r);
        for (int c = x1; c <= x2; c++)
            write(ch);
...
idx_t gfxOut::startCursor(navRoot &root, idx_t x, idx_t y, bool charEdit, idx_t panelNr) {
    ...
    rect(panelNr, x, y, 1, 1, editBgColor, true, enabledStatus, charEdit);
    setColor(cursorColor, editBgColor, true, enabledStatus, charEdit);
    return 0;
}

idx_t gfxOut::endCursor(navRoot &root, idx_t x, idx_t y, bool charEdit, idx_t panelNr) {
    setColor(fgColor, true, enabledStatus, false);
    return 0;
}

idx_t gfxOut::editCursor(navRoot &root, idx_t x, idx_t y, bool editing, bool charEdit, idx_t panelNr) {
    // TODO: next version, this function should return an USED value (and avoid nasty x-1)
...
#ifdef MENU_DEBUG
    if (!f)
        MENU_DEBUG_OUT << "idleFunc is NULL!!!" << endl;
#endif
    if (!f)
        return proceed;
...
Used menuOut::printMenu(navNode &nav, idx_t panelNr) {
    ...
    if (!(nav.root->navFocus->has((systemStyles)(_parentDraw | _menuData)))) {
        ...
        idx_t st = ((nav.target->style() & showTitle) || (nav.root->showTitle && (!(nav.target->style() & noTitle)))) &&
        ...
        bool all = (style & redraw) || (tops[topi] != ot) || (drawn != nav.target) || (panels.nodes[panelNr] != &nav);
