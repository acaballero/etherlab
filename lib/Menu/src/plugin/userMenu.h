...
class UserMenu : public menuNode {
  public:
    ...
    UserMenu(constMEM menuNodeShadow &shadow, const char *backTitle = NULL) : menuNode(shadow), backTitle(backTitle) {}
    UserMenu(constMEM menuNodeShadow &shadow, menu &subMenu, const char *backTitle = NULL) : menuNode(shadow), subMenu(&subMenu), backTitle(backTitle) {}
    ...
    Used printTo(navRoot &root, bool sel, menuOut &out, idx_t idx, idx_t len, idx_t p) override {
        if (root.navFocus != this)
            return menuNode::printTo(root, sel, out, idx, len, p);
        if (idx < 0)
            return menuNode::printTo(root, sel, out, idx, len, p); // print the title
        if (backTitle && idx == sz() - 1)
...
template <typename T> class StdMenu : UserMenu {
    T &src;
    StdMenu(constText *text, T &src, action a = noAction, eventMask e = noEvent, styles style = wrapStyle) : menuNode(text, 0, NULL, a, e), src(src) {}
