...
#include "corePort/Stream.h"
#include "menuBase.h"
#include "shadows.h"
#include <stdint.h>

namespace Menu {
// inline menuOut::styles operator | (menuOut::styles a, menuOut::styles b)
// {return (menuOut::styles)(a|b);}
...
class menuOut : public Print {
 ...
 public:
   ...
   enum styles {
      ...
      redraw =
          1 << 0, // changing one part implies printing all visible (deprecated)
      ...
      expandEnums = 1 << 4, // enumerated typed should be printed along with the
   ...
   virtual menuOut &fill(int x1, int y1, int x2, int y2, char ch = ' ',
                         colorDefs color = bgColor, bool selected = false,
                         status stat = enabledStatus, bool edit = false);

   void
   clearChanged(navNode &nav); // clean up changed flags after everyone printed!
   void previewMenu(navRoot &root, menuNode &menu,
   ...
   virtual void clearLine(idx_t ln, idx_t panelNr = 0,
                          colorDefs color = bgColor, bool selected = false,
                          status stat = enabledStatus, bool edit = false) = 0;
   ...
   virtual void setColor(colorDefs c, colorDefs b, bool selected = false,
                         status s = enabledStatus, bool edit = false);

   virtual void setColor(colorDefs c, bool selected = false,
                         status s = enabledStatus, bool edit = false);

   virtual void drawCursor(idx_t ln, bool selected, status stat,
                           bool edit = false, idx_t panelNr = 0);

   // text editor cursors
   virtual idx_t startCursor(navRoot &root, idx_t x, idx_t y, bool charEdit,
                             idx_t panelNr = 0);

   virtual idx_t endCursor(navRoot &root, idx_t x, idx_t y, bool charEdit,
                           idx_t panelNr = 0);

   virtual idx_t editCursor(navRoot &root, idx_t x, idx_t y, bool editing,
                            bool charEdit, idx_t panelNr = 0);

   virtual void rect(idx_t panelNr, idx_t x, idx_t y, idx_t w = 1, idx_t h = 1,
                     colorDefs c = bgColor, bool selected = false,
                     status stat = enabledStatus, bool edit = false);

   virtual void box(idx_t panelNr, idx_t x, idx_t y, idx_t w = 1, idx_t h = 1,
                    colorDefs c = bgColor, bool selected = false,
                    status stat = enabledStatus, bool edit = false);

#ifdef MENU_FMT_WRAPS
   virtual result fmtStart(prompt &target, fmtParts part, navNode &nav,
                           idx_t idx = -1);
   virtual result fmtEnd(prompt &target, fmtParts part, navNode &nav,
                         idx_t idx = -1);
#endif
#ifdef MENU_ASYNC
...
class cursorOut : public menuOut {
 public:
   ...
   void clearLine(idx_t ln, idx_t panelNr = 0, colorDefs color = bgColor,
                  bool selected = false, status stat = enabledStatus,
                  bool edit = false) override;

   void clear(idx_t panelNr) override;

   menuOut &fill(int x1, int y1, int x2, int y2, char ch = ' ',
                 colorDefs color = bgColor, bool selected = false,
                 status stat = enabledStatus, bool edit = false) override;
};
...
class gfxOut : public menuOut {
 public:
   ...
   inline gfxOut(idx_t rx, idx_t ry, idx_t *t, panelsList &p,
                 menuOut::styles st = menuOut::minimalRedraw,
                 idx_t fontMarginY = 1)
       : menuOut(t, p, st), resX(rx), resY(ry), fontMarginY(fontMarginY) {}

   idx_t startCursor(navRoot &root, idx_t x, idx_t y, bool charEdit,
                     idx_t panelNr) override;

   idx_t endCursor(navRoot &root, idx_t x, idx_t y, bool charEdit,
                   idx_t panelNr) override;

   idx_t editCursor(navRoot &root, idx_t x, idx_t y, bool editing,
                    bool charEdit, idx_t panelNr) override;
};

#ifdef MENU_ASYNC
template <typename O> class asyncOut : public O {
   using O::O;
