...
xpt2046_two_dimension_t xpt2046_clip_touch_coordinate(xpt2046_two_dimension_t coord,
                                                      xpt2046_two_dimension_t min, xpt2046_two_dimension_t max);

xpt2046_two_dimension_t xpt2046_project_touch_coordinate(xpt2046_t *lcd,
                                                         uint16_t x_pos, uint16_t y_pos);


xpt2046_t *xpt2046_touch_init(
    ...
    lcd->touch_coordinate = (xpt2046_two_dimension_t) {{0U},
                                                       {0U}};
    lcd->touch_calibration = itcScalar;
    lcd->touch_scalar = (xpt2046_scalar_calibrator_t) {{{150},  {200}},
                                                       {{1960}, {1960}}};
    lcd->touch_3point = (xpt2046_3point_calibrator_t) {{{0U}, {0U}},
                                                       0,
                                                       0,
                                                       0.0F,
                                                       0.0F,
...
xpt2046_touch_pressed_t xpt2046_touch_pressed(xpt2046_t *lcd) {
    if (NULL == lcd) { return itpNONE; }

    if (GPIO_PIN_RESET == HAL_GPIO_ReadPin(XPT2046_TOUCH_IRQ_PORT, XPT2046_TOUCH_IRQ_PIN)) { return itpPressed; }
    else { return itpNotPressed; }
}
...
xpt2046_touch_pressed_t xpt2046_touch_coordinate(xpt2046_t *lcd,
                                                 uint16_t *x_pos, uint16_t *y_pos) {
    ...
    if (num_samples < req_samples) { return itpNotPressed; }
...
void xpt2046_calibrate_scalar(xpt2046_t *lcd,
                              uint16_t min_x, uint16_t min_y, uint16_t max_x, uint16_t max_y) {
    if (NULL == lcd) { return; }

    lcd->touch_calibration = itcScalar;
    lcd->touch_scalar.min = (xpt2046_two_dimension_t) {{min_x},
                                                       {min_y}};
    lcd->touch_scalar.max = (xpt2046_two_dimension_t) {{max_x},
                                                       {max_y}};
}

void xpt2046_calibrate_3point(xpt2046_t *lcd,
    ...
    lcd->touch_3point.scale = (xpt2046_two_dimension_t) {{scale_width},
                                                         {scale_height}};
    ...
    lcd->touch_3point.alpha_x =
            (float) (((screen_a_x - screen_c_x) * (touch_b_y - touch_c_y))
                     -
                     ((screen_b_x - screen_c_x) * (touch_a_y - touch_c_y))) / delta;

    lcd->touch_3point.beta_x =
            (float) (((touch_a_x - touch_c_x) * (screen_b_x - screen_c_x))
                     -
                     ((touch_b_x - touch_c_x) * (screen_a_x - screen_c_x))) / delta;

    lcd->touch_3point.delta_x =
            (float) ((((int64_t) screen_a_x
...
static int32_t interp(int32_t x, int32_t x0, int32_t x1, int32_t y0, int32_t y1) {
    if (x1 == x0) { return 0; } // return 0 on divide-by-zero
...
xpt2046_two_dimension_t xpt2046_project_touch_coordinate(xpt2046_t *lcd, uint16_t x_pos, uint16_t y_pos) {
    xpt2046_two_dimension_t coord = (xpt2046_two_dimension_t) {{x_pos},
                                                               {y_pos}};
            ...
            case itcScalar:
                ...
                if (false) { // calibrate
                    ...
                    coord = xpt2046_clip_touch_coordinate(
                            {{coord.x},
                             {coord.y}},
                            {{lcd->touch_scalar.min.x},
                             {lcd->touch_scalar.min.y}},
                            {{lcd->touch_scalar.max.x},
                             {lcd->touch_scalar.max.y}});
                }
...
uint16_t xpt2046_read_bitbang(uint8_t command) {
    ...
    for (int i = 7; i >= 0; i--) {
        HAL_GPIO_WritePin(XPT2046_TOUCH_DATA_IN_PORT, XPT2046_TOUCH_DATA_IN_PIN,
                          static_cast<GPIO_PinState>(command & (1 << i)));
        HAL_GPIO_WritePin(XPT2046_TOUCH_CLK_PORT, XPT2046_TOUCH_CLK_PIN, GPIO_PIN_SET);
        delay_us(10);
        HAL_GPIO_WritePin(XPT2046_TOUCH_CLK_PORT, XPT2046_TOUCH_CLK_PIN, GPIO_PIN_RESET);
        delay_us(10);
