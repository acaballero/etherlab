...
void Display::drawArea(Area *area, Painter *painter, bool pad_display) {
    ...
    if (this->enabled) {
        ...
        uint16_t dma_buffer_size =
                buffer_size_pixels_remaining < max_buffer_size ? buffer_size_pixels_remaining : max_buffer_size;
...
void Display::setPixel(uint16_t x, uint16_t y, uint16_t c) {
    ...
    // So, if we have to fill a area, use the buffer (getBuffer) instead with incremental offset
    Area *area = this->curr_area;
    ...
    if (y >= this->current_line && y < this->current_line + this->chunk_height) {
        ...
        uint16_t ix = ((dy * area->width) + x); // >> buffer->log2_pixels_per_byte;
        ...
        if (yLonger) {
            if (longLen > 0) {
                ...
                for (int j = 0x8000 + (x1 << 16); ipy <= longLen; ++ipy) {
                    if (ipy >= zy1 && ipy < zy2) {
                        ipx = (ipy - this->current_line) * area->width; // offset in the buffer
            ...
            for (i = si; i < ei; i++, dy++) {
                ...
                for (j = j1, px = x; j < jn; j++, px++) {
                    ...
                    if (c != C565_TRANSPARENT) {
                        uint16_t ix = ((dy * area->width) + px); // >> buffer->log2_pixels_per_byte;
